...
export const GameCard: React.FC = () => {
  const { currentItem, handleGuess, gameState, selectedAnswer, nextRound } = useGame();
    ...
    <div className="w-full max-w-4xl mx-auto flex flex-col gap-8">
      ...
      >
        <div className="mb-4 flex justify-between items-center text-sm font-semibold uppercase tracking-wider text-muted-foreground">
          <span className="bg-primary/10 text-primary px-3 py-1 rounded-full">{currentItem.type}</span>
          <span className={cn("px-3 py-1 rounded-full border", 
            currentItem.difficulty === 'Easy' ? 'border-green-500/50 text-green-500' : 
            currentItem.difficulty === 'Medium' ? 'border-yellow-500/50 text-yellow-500' : 
        ...
        <AnimatePresence mode="wait">
          {!isRevealed ? (
            ...
            <motion.div 
              ...
              className={cn("p-8 rounded-3xl shadow-2xl border-2 flex flex-col md:flex-row gap-8 items-center",
                isCorrect ? "bg-green-500/10 border-green-500 dark:border-green-400" : "bg-red-500/10 border-red-500 dark:border-red-400"
              )}
            >
              ...
              <div className="flex-grow text-center md:text-left">
                <h3 className={cn("text-3xl font-bold mb-2", isCorrect ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400")}>
                  {isCorrect ? 'Correct!' : 'Wrong!'}
                </h3>
                <p className="text-xl mb-4">
                  This was created by <strong className="font-bold">{currentItem.answer === 'AI' ? 'an AI' : 'a Human'}</strong>.
                </p>
