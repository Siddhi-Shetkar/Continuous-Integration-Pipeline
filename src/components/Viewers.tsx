...
import { Play, Pause, Maximize, ZoomIn } from 'lucide-react';
...
const ImageViewer = ({ src }: { src: string }) => {
  return (
    <div className="relative group overflow-hidden rounded-xl bg-black/5 flex items-center justify-center min-h-[300px]">
      <img src={src} alt="Guess the source" className="object-cover max-h-[500px] w-full h-full transition-transform duration-500 group-hover:scale-105" />
...
const ArtworkViewer = ({ src }: { src: string }) => {
  return (
    <div className="relative group overflow-hidden rounded-xl bg-black/5 flex items-center justify-center min-h-[400px] p-4 border-8 border-white dark:border-gray-800 shadow-2xl">
      <img src={src} alt="Artwork" className="object-contain max-h-[500px] w-full transition-transform duration-700 group-hover:scale-110" />
    </div>
  );
};
      ...
      <div className="p-4 overflow-x-auto">
        {lines.map((line, i) => (
          <div key={i} className="flex">
            <span className="text-gray-500 w-8 select-none text-right mr-4">{i + 1}</span>
...
const VoiceViewer = ({ url }: { url: string }) => {
      ...
      <div className="flex items-center gap-1 h-12 w-full max-w-md">
        {Array.from({ length: 40 }).map((_, i) => (
          <motion.div 
            ...
            animate={{ height: isPlaying ? ['20%', '100%', '40%', '80%', '20%'] : '10%' }}
            transition={{ repeat: Infinity, duration: 0.5 + Math.random() * 0.5, ease: "linear" }}
          />
        ))}
      </div>
      <p className="mt-4 font-medium opacity-80">Click to listen to the sample</p>
    </div>
  );
};
