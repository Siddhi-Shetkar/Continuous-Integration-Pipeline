...
textHuman.forEach((text, i) => {
  data.push({
    ...
    content: text,
    explanation:
      'Contains personal anecdotes, colloquialisms, and genuine emotion typical of human writing.',
  })
})
...
textAI.forEach((text, i) => {
  data.push({
    ...
    content: text,
    explanation:
      'Highly objective, perfectly structured, and overly generic tone points to AI generation.',
  })
})
...
codeHuman.forEach((code, i) => {
  data.push({
    ...
    content: code,
    explanation:
      'Quirky comments, slightly messy logic, and humorous notes are classic human traits.',
  })
})
...
codeAI.forEach((code, i) => {
  data.push({
    ...
    content: code,
    explanation:
      'Perfect JSDoc comments, rigorous error checking, and sterile naming conventions.',
  })
})
...
for (let i = 0; i < 20; i++) {
  data.push({
    id: id++,
    type: 'image',
    answer: i % 2 === 0 ? 'Human' : 'AI',
    difficulty: difficulties[i % 3],
    content: `https://picsum.photos/seed/${i + 100}/500/300`,
    explanation:
      i % 2 === 0
        ? 'Natural lighting and cohesive physics.'
        : 'Slight inconsistencies in the background elements.',
  })
}
...
for (let i = 0; i < 20; i++) {
  data.push({
    ...
    explanation:
      i % 2 === 0
        ? 'Intentional brushstrokes and emotion.'
        : 'Overly smooth blending and slightly illogical geometry.',
  })
}
  ...
  data.push({
    ...
    explanation:
      i % 2 === 0
        ...
        : 'Consistent tone without natural variations.',
  })
}
